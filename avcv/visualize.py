...
def bbox_visualize(
    img, boxes, scores, cls_ids, conf=0.5, class_names=None, texts=None, box_color=None
):
    img = img.copy()
    for i in range(len(boxes)):
        ...
        if box_color is None:
            ...
            _color = np.array(box_color) / 255.0
    ...
    def draw(self):
        for i, text in enumerate(self.texts):
            ...
            if isinstance(text, tuple):
                ...
                cv2.putText(
                    self.board, text, (0, p2[1]), cv2.FONT_HERSHEY_COMPLEX, 1, color
                )
                ...
                y = (i + 1) * self.line_h
                cv2.putText(
                    self.board, text, (x, y), cv2.FONT_HERSHEY_COMPLEX, 1, color
                )
        return self.board
